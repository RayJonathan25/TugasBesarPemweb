<html>
	...
	<body>
		...
		<form id = "formRegister" method="post" onsubmit="return submitRegister()" >
			<input type="email" name="email" id="input" class="email" placeholder="email" /> <p></p>
			
			<input type="text" name="username" id="input" class="username" placeholder="username" onkeyup="showHint(this.value, 'usernameHints.php', 'txtHintUsername')"/> 
			<p style='color:red;' id="txtHintUsername"></p>
			
			<input type="password" name="password" id="input" class="password" placeholder="password (min: 8 characters)" onkeyup="showHint(this.value, 'passwordHints.php', 'txtHintPassword')" />  
